...
from app.api.routes.prompts import analyze_prompt
...
async def seed_database() -> int:
    ...
    # Clear PostgreSQL records first to prevent duplicates
    db = SessionLocal()
    try:
        db.query(PromptRecordDb).delete()
        # Seed mock users first so foreign keys pass
        mock_users = {
            "dev1": {"email": "alice@promptiq.dev", "name": "Alice Chen", "picture": "https://lh3.googleusercontent.com/a/default-user-alice"},
            "dev2": {"email": "bob@promptiq.dev", "name": "Bob Kumar", "picture": "https://lh3.googleusercontent.com/a/default-user-bob"},
        }
        for uid, info in mock_users.items():
            user_exists = db.query(User).filter(User.id == uid).first()
            if not user_exists:
                db_user = User(id=uid, email=info["email"], name=info["name"], picture=info["picture"])
                db.add(db_user)
        db.commit()
    except Exception as exc:
        logger.warning("Seeding clean/user step failed: %s", exc)
    ...
    for prompt_info in SEED_PROMPTS:
        try:
            ...
            response = await analyze_prompt(req, db=db)
            
            # Backdate in the database record so it builds a realistic timeline
            ...
            if db_record:
                ...
                db_record.created_at = backdated_time
                db.commit()
