...
function buildHeaders(apiKey?: string): HeadersInit {
  ...
  if (apiKey) {
    // If an explicit key is provided (e.g. from CLI or custom headers), use it
    headers["Authorization"] = `Bearer ${apiKey}`;
  } else if (typeof window !== "undefined") {
    // Automatically retrieve JWT token from localStorage in browser context
  ...
  async getDashboardStats(apiKey?: string): Promise<DashboardStats> {
    ...
    return {
      ...
      model_distribution: val.model_distribution.map((m: any) => ({
        ...
        color: m.model.includes("claude") ? "#ef4444" : "#8B5CF6"
      })),
