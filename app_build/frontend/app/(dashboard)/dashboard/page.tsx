...
const MOCK_MODEL_DIST = [
  ...
  { model: "Claude Opus", count: 189, percentage: 27, color: "#ef4444" },
...
export default function DashboardPage() {
  ...
  const displayStats = hasData ? stats : {
    ...
    total_cost: 0.0,
    total_savings: 0.0,
    avg_necessity_score: 0.0,
    active_users: 0,
    model_distribution: [],
  };

  const displayPrompts = recentPromptsData;
  const displayTrend = trendDataList;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Developer Onboarding Card (displays when no prompts are recorded) */}
      {!hasData && !loading && (
        <div className="glass-card p-8 border border-red-500/20 bg-[#0d0505]/40 rounded-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-red-500/5 rounded-full blur-3xl pointer-events-none" />
          <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-red-500" />
            Connect Your IDE to Begin Routing
          </h2>
          <p className="text-sm text-[var(--text-secondary)] max-w-2xl mb-6">
            PromptIQ is ready! To start collecting analytics, analyzing necessity scores, and routing queries to the most cost-effective models, connect your IDE proxy client or use the CLI package with the instructions below.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-black/40 border border-white/5 rounded-xl p-4 font-mono text-xs text-slate-300 space-y-2">
              <p className="text-red-400 font-bold">// 1. Install CLI</p>
              <p className="text-slate-500">npm install -g promptiq-cli</p>
              <p className="text-slate-500">promptiq link --host http://curiositybytejas.cloud:8000</p>
            </div>
            <div className="bg-black/40 border border-white/5 rounded-xl p-4 font-mono text-xs text-slate-300 space-y-2">
              <p className="text-red-400 font-bold">// 2. Configure IDE proxy</p>
              <p className="text-slate-500">API Key: promptiq-dev-key-change-me</p>
              <p className="text-slate-500">Settings: Enable local Llama3 routing via Ollama</p>
            </div>
          </div>
        </div>
      )}
