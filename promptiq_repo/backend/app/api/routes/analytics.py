...
async def get_dashboard_stats(
    ...
    Pulls data from Cognee memory and the local prompt store.
    """
    ...
    # Get prompt records from local store
    from app.api.routes.prompts import _prompt_store

    records = list(_prompt_store.values())

    # Compute stats from local store
    total_prompts = len(records)
    total_cost = sum(r.analysis.estimated_ai_cost_usd for r in records)
    total_time_saved = sum(r.analysis.estimated_manual_minutes for r in records)
    avg_complexity = (
        sum(r.analysis.complexity_score for r in records) / max(total_prompts, 1)
    )
    avg_necessity = (
    ...
    for r in records:
        dom = r.analysis.skill_domain.value
    ...
    # Active users
    unique_users = set(r.prompt_data.user_id for r in records)
...
async def get_cost_analytics(
    ...
    for r in records:
        model = r.recommended_model or "unknown"
        cost_by_model[model] = cost_by_model.get(model, 0) + r.analysis.estimated_ai_cost_usd

    # Cost by user
    cost_by_user: Dict[str, float] = {}
    for r in records:
        user = r.prompt_data.user_id
        cost_by_user[user] = cost_by_user.get(user, 0) + r.analysis.estimated_ai_cost_usd

    # Cost by project
    cost_by_project: Dict[str, float] = {}
    for r in records:
        project = r.prompt_data.project
        cost_by_project[project] = cost_by_project.get(project, 0) + r.analysis.estimated_ai_cost_usd

    # Cost by category
    cost_by_category: Dict[str, float] = {}
    for r in records:
        cat = r.analysis.category.value
        cost_by_category[cat] = cost_by_category.get(cat, 0) + r.analysis.estimated_ai_cost_usd

    total_cost = sum(r.analysis.estimated_ai_cost_usd for r in records)

    # Estimate savings: cost if all used premium model vs actual
    premium_cost = sum(
        (r.analysis.token_count / 1000.0) * 0.015 * 2.5  # rough premium estimate
        for r in records
    )
    ...
    for r in records:
        ...
        day_costs[day] = day_costs.get(day, 0) + r.analysis.estimated_ai_cost_usd
    cost_trend = [
